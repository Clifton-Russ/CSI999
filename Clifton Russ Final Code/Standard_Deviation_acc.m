...
load std_results_acc.mat;

%% This collects all the NB models for accuracy
acc_NB = cell(3,4);
temp = {A1_NB1, A2_NB1, A3_NB1, A4_NB1; ...
        A1_NB2, A2_NB2, A3_NB2, A4_NB2; ...
        A1_PCA_NB, A2_PCA_NB, A3_PCA_NB, A4_PCA_NB};

% For loop to determine std for NB (Accuracy) 
x = "ClassificationNaiveBayes";

for i = 1:size(temp,1)
    for j = 1:size(temp,2)
        acc_NB{i,j} = determine_std(cell2mat(temp(i,j)), x);
    end
end

%% This collects all the SVM models for accuracy
acc_svm = cell(3,4);
temp = {A1_SVM1, A2_SVM1, A3_SVM1, A4_SVM1; ...
        A1_SVM2, A2_SVM2, A3_SVM2, A4_SVM2; ...
        A1_PCA_SVM, A2_PCA_SVM, A3_PCA_SVM, A4_PCA_SVM};

% For loop to determine std for SVM (Accuracy) 
x = "ClassificationSVM";

for i = 1:size(temp,1)
    for j = 1:size(temp,2)
        acc_svm{i,j} = determine_std(cell2mat(temp(i,j)), x);
    end
end

%% This collects all the KNN models for accuracy
acc_KNN = cell(4,4);
...
for i = 1:size(temp,1)
    for j = 1:size(temp,2)
        acc_KNN{i,j} = determine_std(cell2mat(temp(i,j)), x);
    end
end

%% This collects all the ANN models for accuracy
acc_ANN = cell(6,4);
...
for i = 1:size(temp,1)
    for j = 1:size(temp,2)
        acc_ANN{i,j} = determine_std(cell2mat(temp(i,j)), x);
    end
end
