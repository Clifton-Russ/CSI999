function [click_percent_step_mean,report_rate, markov, transition_matrix] = create_suborganization(a_size_total, days, agent_compliance_average,incident_threshold, phish_report_rate, scenario)
...
if scenario == "1a"
    people = create_agent2(1,a_size_total,agent_compliance_average,threat_size);
end
...
% the following scenarios changes the environmental compliance in groups of
% .25.
if scenario == "1b2"
...
vuln_breach_total = cell(a_size,days);
    for i = 1:a_size
        for j = 1: days
             %[vuln_controls{i,j}, ~, incident_input{i,j}] = determine_state_vuln6(agent_compliance_average, mc_baseline{i,j},e_thresholds_daily{1,j},vuln_idx);
             ...
             %[vuln_controls{i,j}, ~, incident_input{i,j}] = determine_state_vuln7(agent_compliance_average, mc_baseline{i,j},e_thresholds_daily{1,j},vuln_idx);
             ...
             if scenario == "1d2"
                ...
                [vuln_controls{i,j}, incident_input{i,j}, vuln_breach{i,j}, ~] = determine_state_vuln8(agent_compliance_average, mc_baseline{i,j},e_thresholds_daily{1,j},vuln_idx);
             end
        end
    end
...
%% Remove activity 5 ("other") in order to deteermine the click rate.
